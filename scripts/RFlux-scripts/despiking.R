despiking <- function(x, mfreq, variant, wsignal, wscale, wby=1, zth=5, alpha=0.01){
		...
		if(N >= mfreq*10 & all(apply(matrix(TS, nrow=mfreq), MARGIN=1, function(x) sum(is.na(x))<(N/mfreq-3)))){
			...
			if(length(TS>(mfreq*10-1)) & all(apply(matrix(TS, nrow=mfreq), MARGIN=1, function(x) length(which(is.na(x)))) < ceiling(N/mfreq))){
				...
				for (i in 1:(length(indq)-1)){
					...
					if(length(na.omit(res[ind]))>9) {asd <- aout.laplace(res[ind], c(median(res[ind], na.rm=TRUE), robustbase::Qn(na.omit(res[ind]))), alpha); spike_index <- c(spike_index, ind[which(asd$is.outlier)])}
	...
	if(variant=="v2"){ ## mainly for meteo time series
		...
		spike_tmp <- c()
		TSd <- c(NA,diff(TS))
		th_local <- as.numeric(zoo::rollapply(as.ts(TSd), width=wscale, by=wby, function(x) zth*robustbase::Qn(na.omit(x)), fill=NA))
		...
		if (length(spike_tmp)>0) {
			for (i in 1:length(spike_tmp)){
				...
				if (spike_tmp[i] > (wsignal-1)/2 & spike_tmp[i] < N-(wsignal-1)/2) {
					...
					res0 <- (TSw-signal$level)[(wsignal+1)/2,];
	...
	if(variant=="v3"){
		...
		mod_rm <- robfilter::rm.filter(x_raw, width=wsignal)
		...
		{res <- c(NA, diff(x_c));
			lbound <- robustbase::Qn(na.omit(res));
			sigma_est_base <- zoo::na.locf(zoo::na.locf(zoo::na.approx(zoo::rollapply(res, width=wscale, by=wby, function(x) robustbase::Qn(x), fill=NA), na.rm=FALSE), na.rm=FALSE), fromLast=TRUE);
			sigma_est <- replace(sigma_est_base, which(sigma_est_base < lbound), lbound)
			},
		{res <- x_c - fit_rm;
		lbound <- max(robustbase::Qn(na.omit(res)),0.01); ## minimum threshold bound for the scale parameter set equal to 0.01
		sigma_est_base <- zoo::na.locf(zoo::na.locf(zoo::na.approx(zoo::rollapply(res, width=wscale, by=wby, function(x) robustbase::Qn(na.omit(x)), fill=NA), na.rm=FALSE), na.rm=FALSE), fromLast=TRUE);
		sigma_est <- replace(sigma_est_base, which(sigma_est_base < lbound), lbound)
		})
